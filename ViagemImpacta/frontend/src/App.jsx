...
// --- TODAS AS IMPORTAÇÕES DE IMAGENS DE HOTÉIS (CORRIGIDAS PARA './assets/images/') ---
// **ATENÇÃO: VERIFIQUE SE OS NOMES DOS ARQUIVOS EM SUA PASTA FÍSICA SÃO EXATOS A ESTES**
...
// **MOVIDO DE HotelsPage.js PARA CÁ** (COM DADOS ATUALIZADOS E CORRIGIDOS)
const allHotelsData = [
  {
    id: 1,
...
const convertDdMmYyyyToYyyyMmDd = (dateString) => {
  ...
  const allAvailableTravels = [...allPromotionalTravels]; // Ajustado para incluir flights
  ...
  const handleBackToList = () => {
    if (currentPage === 'flightDetails') {
      ...
      setPromotionToPurchase(null);
        ...
        setCurrentPage('home'); // Volta para a Home, onde está a lista de posts
        setSelectedBlogPostId(null); // Limpa o post selecionado
        setSelectedBlogCategory(null); // Limpa a categoria selecionada
    } else if (currentPage === 'blogCategory') { // Adiciona condição para voltar da página de categoria
        setCurrentPage('home');
        setSelectedBlogCategory(null); // Limpa a categoria selecionada
    } else if (currentPage === 'recommendedHotelDetails') { // NOVO: Volta da RecommendedHotelDetails para a Home
        setCurrentPage('home');
  ...
  const handleNavigateToHome = () => {
    ...
    setSelectedFlightId(null);
    setActiveFilterButton('');
    setSelectedBlogCategory(null); // Limpa a categoria ao voltar para a home
    setSelectedRecommendedHotelId(null); // Limpa hotel recomendado ao voltar para a home
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  // Função para navegar para a página Institucional
  const handleNavigateToInstitutional = () => {
    setCurrentPage('institutional');
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // Função para lidar com o clique em um post de blog
  const handleBlogPostClick = (postId) => {
    setSelectedBlogPostId(postId); // Define o ID do post clicado
    setCurrentPage('blogPostDetails'); // Muda para a nova página de detalhes
    setSelectedBlogCategory(null); // Garante que a categoria filtrada seja limpa
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // NOVA FUNÇÃO: Lida com o clique na categoria do blog
  const handleBlogCategoryClick = (category) => {
    setSelectedBlogCategory(category); // Define a categoria selecionada
    setCurrentPage('blogCategory'); // Muda para uma nova "página" de categoria
    setSelectedBlogPostId(null); // Limpa o post selecionado, se houver
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const handleRecommendedHotelDetailsClick = (hotelId) => {
    ...
    setCurrentPage('recommendedHotelDetails'); // Nova página para detalhes de recomendados
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  // Filtra os posts do blog com base na categoria selecionada
  const filteredBlogPosts = selectedBlogCategory
    ? blogPosts.filter(post => post.category === selectedBlogCategory)
    : blogPosts; // Se nenhuma categoria estiver selecionada, mostra todos
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-grow">
        ...
        {/* A seção de botões de categoria ainda é condicional */}
        {currentPage !== 'login' && currentPage !== 'register' && currentPage !== 'myTravels' && currentPage !== 'institutional' && currentPage !== 'recommendedHotelDetails' && ( // Excluir RecommendedHotelDetails também
        ...
        {selectedFlightId ? (
          ...
          <p className="text-center py-10 text-xl font-semibold">Minhas Viagens e Hotéis Salvos (implementar MyTravelsPage)</p>
        ) : currentPage === 'hotels' ? (
          <HotelsPage
        ...
        ) : currentPage === 'recommendedHotelDetails' && selectedRecommendedHotelId ? ( // NOVO: Renderiza detalhes do hotel recomendado
            <RecommendedHotelDetailsPage
                hotel={allHotelsData.find(h => h.id === selectedRecommendedHotelId)}
                onBack={handleBackToList}
            />
        ) : (
          <>
            ...
            />
            {/* NOVO: Seção de Hotéis Mais Bem Avaliados */}
            <TravelSection
              id="recomendado-viajantes"
              title="Recomendado por Viajantes: Nossos Melhores Hotéis"
              travels={allHotelsData.filter(hotel => hotel.rating && hotel.rating >= 4.5).sort((a, b) => b.rating - a.rating)} // Filtra e ordena
              onCardClick={handleRecommendedHotelDetailsClick} // Nova função de clique
              onSaveTravel={handleSaveHotel} // Mantém save para hotéis
              isHotelSaved={isHotelSaved} // Mantém isSaved para hotéis
              CardComponent={RecommendedHotelCard} // Usa o novo card
            />
            <BlogSection
              id="dicas-de-viagem"
